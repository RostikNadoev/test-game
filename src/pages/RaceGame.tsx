...
export const RaceGame: React.FC = () => {
  ...
  const updateEngineSound = (currentSpeed: number) => {
    if (!oscillator.current || !audioCtx.current || !gainNode.current) return;
    const freq = 40 + (Math.abs(currentSpeed) * 15);
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      setSpeed(Math.floor(Math.abs(c.speed) * 42));
  ...
  const handleJoystick = (e: React.TouchEvent | React.MouseEvent, type: string) => {
    initAudio();
  ...
  return (
    <div 
      ref={containerRef} 
      className="fixed inset-x-0 bottom-0 overflow-hidden bg-[#0A0A0F] touch-none select-none font-mono text-white"
    ...
    >
      ...
      <div 
        ...
        onMouseUp={() => handleJoystick({} as any, 'end')}

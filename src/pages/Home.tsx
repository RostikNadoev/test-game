...
const games = [
  ...
  { id: 'paper', name: 'Paper Duel', icon: '🔲', description: 'Захват территорий', color: 'from-emerald-500 to-teal-700' },
...
export const Home = () => {
  ...
  return (
    <div className="p-4 pb-20">
      <div className="mb-6">
        <h1 className="text-2xl font-bold bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent">TwinGames</h1>
        <p className="text-gray-400 text-sm">Выбери битву и найди соперника</p>
      </div>
      <div className="grid grid-cols-2 gap-4">
        {games.map((game) => (
          <GameCard key={game.id} game={game} onClick={() => navigate(`/game/${game.id}/lobbies`)} />
        ))}
      </div>
    </div>
  );

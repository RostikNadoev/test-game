...
export const ArcherGame: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const drawMoon = (ctx: CanvasRenderingContext2D, w: number) => {
        ...
        ctx.fillStyle = '#e2e8f0'; ctx.shadowBlur = 0; ctx.globalAlpha = 0.4;
        ctx.beginPath(); ctx.arc(mx - 10, my - 10, 8, 0, Math.PI * 2); ctx.fill();
        ctx.beginPath(); ctx.arc(mx + 15, my + 5, 5, 0, Math.PI * 2); ctx.fill();
        ctx.restore();
    };
    ...
    const drawAimSystem = (ctx: CanvasRenderingContext2D, p: Player, h: number) => {
      ...
      for (let i = 0; i < 80; i++) {
        ...
        tvx += wind * 0.005; tvy += SETTINGS.gravity; tx += tvx; ty += tvy;
    ...
    const drawStylizedPlayer = (ctx: CanvasRenderingContext2D, p: Player) => {
      ...
      ctx.beginPath(); ctx.arc(p.x + 25 + lookDir, p.y + 3, 3, 0, Math.PI * 2); ctx.arc(p.x + 25 + lookDir + (lookDir > 0 ? 8 : -8), p.y + 3, 3, 0, Math.PI * 2); ctx.fill();
      ctx.restore();
    };
    ...
    const drawLushGrass = (ctx: CanvasRenderingContext2D, h: number) => {
      ...
      grassBlades.current.forEach(b => {
        ...
        ctx.quadraticCurveTo(b.x, groundPos - b.h, b.x + Math.sin(time + b.offset) * 4, groundPos - b.h); ctx.stroke();
      });
    };
    ...
    const handleImpact = (pr: any, isHit: boolean) => {
      ...
      if (isHit) {
        camera.current.shake = 40; 
        createExplosion(pr.x, pr.y, '#ff0000', 70, true); // Blood
        if (turn === 1) setP2(p => ({ ...p, hp: Math.max(0, p.hp - 1) }));
        else setP1(p => ({ ...p, hp: Math.max(0, p.hp - 1) }));
      } else {
        createExplosion(pr.x, pr.y, '#fbbf24', 15, false); // Sparks on ground
      }
      setTimeout(() => {
    ...
    const createExplosion = (x: number, y: number, color: string, count: number, isBlood: boolean) => {
      for (let i = 0; i < count; i++) {
        particles.current.push({ 
            x, y, color, 
            size: isBlood ? Math.random() * 6 + 2 : Math.random() * 3, 
            vx: (Math.random() - 0.5) * (isBlood ? 7 : 12), 
            vy: (Math.random() - 0.7) * (isBlood ? 9 : 12), 
            alpha: 1, 
            life: isBlood ? 0.006 : 0.02 
        });
      }
    };
  ...
  const updateWind = () => setWind(Number((Math.random() * 0.06 - 0.03).toFixed(3)));
  ...
  const handleMove = (e: any) => { if (drag.current.active) { const t = e.touches ? e.touches[0] : e; drag.current.currX = t.clientX; drag.current.currY = t.clientY; } };
  const handleEnd = () => {
  ...
  return (
    <div ref={containerRef} className="w-full h-[calc(100vh-164px)] bg-[#020617] relative touch-none overflow-hidden select-none font-sans">
      <div className="absolute inset-0 p-8 flex justify-between items-start z-10 pointer-events-none">
        ...
        <div className={`flex flex-col gap-2 transition-all duration-500 ${turn === 1 ? 'scale-110' : 'opacity-40'}`}>
          ...
          <div className="text-white font-black italic text-[10px] uppercase tracking-tighter [writing-mode:vertical-lr] pl-1">P1_Striker</div>
        </div>

        {/* TOP CENTER: WIND (Moved down below Moon glow) */}
        <div className="flex flex-col items-center gap-4 mt-44"> {/* WAS mt-16, now mt-44 for better spacing */}
        ...
        <div className={`flex flex-col items-end gap-2 transition-all duration-500 ${turn === 2 ? 'scale-110' : 'opacity-40'}`}>
          ...
          <div className="text-white font-black italic text-[10px] uppercase tracking-tighter [writing-mode:vertical-lr] pr-1">P2_Vanguard</div>
        </div>
      </div>

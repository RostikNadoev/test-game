...
func (h LobbyHandler) Active(c *gin.Context) {
	game := realtime.NormalizeGameCode(
		c.Query("game"),
	)

	if game != "" &&
		!services.IsPvpGameSupported(game) {
		c.JSON(
			http.StatusBadRequest,
			gin.H{

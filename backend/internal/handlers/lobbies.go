...
func wsURLForGame(game string, lobbyID string) string {
	...
	case "dunk_shot":
		return "/ws/dunk-shot/" + lobbyID
	case "grid_lock":
		return "/ws/grid-lock/" + lobbyID
	case "flappy_race":
		return "/ws/flappy-race/" + lobbyID
	case "doodle_jump":
		return "/ws/doodle-jump/" + lobbyID
	case "crossy_pvp":
		return "/ws/crossy-road/" + lobbyID
	case "draw_drop":
		return "/ws/draw-drop/" + lobbyID
	case "neon_matrix":
...
func (h LobbyHandler) Leave(c *gin.Context) {
	...
	if lobby != nil {
		response["lobby"] = h.Hub.BuildDTO(lobby)
	}

...
type PhysicsDuelWSHandler struct {
	Cfg        *config.Config
	LobbyStore *realtime.Hub
	Manager    *physicsduel.Manager
}

func (h PhysicsDuelWSHandler) Connect(c *gin.Context) {
	if h.Cfg == nil || h.LobbyStore == nil || h.Manager == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "physics duel websocket is not configured"})
		return
	}
	...
	if realtime.NormalizeGameCode(lobby.Game) != physicsduel.GameCode {
		c.JSON(http.StatusBadRequest, gin.H{"error": "lobby is not descent_duel"})
		return
	}

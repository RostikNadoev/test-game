...
type AirHockeyWSHandler struct {
	Cfg        *config.Config
	LobbyStore *realtime.Hub
	Manager    *airhockey.Manager
}

func (h AirHockeyWSHandler) Connect(c *gin.Context) {
	if h.Cfg == nil || h.LobbyStore == nil || h.Manager == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "air hockey websocket is not configured"})
		return
	}
	...
	if realtime.NormalizeGameCode(lobby.Game) != airhockey.GameCode {
		c.JSON(http.StatusBadRequest, gin.H{"error": "lobby is not air_hockey"})
		return
	}

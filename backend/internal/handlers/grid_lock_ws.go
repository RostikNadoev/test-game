...
type GridLockWSHandler struct {
	Cfg        *config.Config
	LobbyStore *realtime.Hub
	Manager    *gridlock.Manager
}

func (h GridLockWSHandler) Connect(c *gin.Context) {
	if h.Cfg == nil || h.LobbyStore == nil || h.Manager == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "grid lock websocket is not configured"})
		return
	}
	...
	if realtime.NormalizeGameCode(lobby.Game) != gridlock.GameCode {
		c.JSON(http.StatusBadRequest, gin.H{"error": "lobby is not grid_lock"})
		return
	}

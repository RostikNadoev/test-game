...
type DunkShotWSHandler struct {
	Cfg        *config.Config
	LobbyStore *realtime.Hub
	Manager    *dunkshot.Manager
}

func (h DunkShotWSHandler) Connect(c *gin.Context) {
	if h.Cfg == nil || h.LobbyStore == nil || h.Manager == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "dunk shot websocket is not configured"})
		return
	}
	...
	if realtime.NormalizeGameCode(lobby.Game) != dunkshot.GameCode {
		c.JSON(http.StatusBadRequest, gin.H{"error": "lobby is not dunk_shot"})
		return
	}

...
type PlinkoWSHandler struct {
	Cfg        *config.Config
	LobbyStore *realtime.Hub
	Manager    *plinko.Manager
}

func (h PlinkoWSHandler) Connect(c *gin.Context) {
	if h.Cfg == nil || h.LobbyStore == nil || h.Manager == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "plinko websocket is not configured"})
		return
	}
	...
	if realtime.NormalizeGameCode(lobby.Game) != plinko.GameCode {
		c.JSON(http.StatusBadRequest, gin.H{"error": "lobby is not plinko_pvp"})
		return
	}

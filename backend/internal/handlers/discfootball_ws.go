...
type DiscFootballWSHandler struct {
	Cfg        *config.Config
	LobbyStore *realtime.Hub
	Manager    *discfootball.Manager
}

func (h DiscFootballWSHandler) Connect(c *gin.Context) {
	if h.Cfg == nil ||
		h.LobbyStore == nil ||
		h.Manager == nil {
		c.JSON(
			...
			gin.H{
				"error": "disc football websocket is not configured",
			},
		)
	...
	if realtime.NormalizeGameCode(lobby.Game) !=
		discfootball.GameCode {
		c.JSON(
			http.StatusBadRequest,
			gin.H{
				"error": "lobby is not disc_football",
			},
		)

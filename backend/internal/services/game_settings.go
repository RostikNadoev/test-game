...
func ReloadGameSettingsCache(db *gorm.DB) error {
	...
	if err := db.Order("sort_order ASC, code ASC").Find(&rows).Error; err != nil {
		return err
	}
	m := make(map[string]models.GameSetting, len(rows))
	for _, row := range rows {
		m[row.Code] = row
	}
	gameSettingsMu.Lock()
	cachedGameSettings = m
	gameSettingsMu.Unlock()
	return nil
}

func GetGameSetting(code string) (models.GameSetting, bool) {
...
func ListGameSettings() []models.GameSetting {
	gameSettingsMu.RLock()
	defer gameSettingsMu.RUnlock()
	out := make([]models.GameSetting, 0, len(cachedGameSettings))
...
func ListEnabledPvpGames() []catalog.PvpGame {
	...
	for _, row := range ListGameSettings() {
		...
		out = append(out, catalog.PvpGame{Code: row.Code, DisplayName: row.Title})
	}
	return out
}

func GetSoloGameConfig(code string) (solo.GameConfig, error) {
...
func UpdateGameSetting(db *gorm.DB, code string, patch map[string]any) (*models.GameSetting, error) {
	code = normalizeGameCode(code)
	...
	if v, ok := patch["min_bet"].(float64); ok && v > 0 {
		row.MinBet = v
	}
	if v, ok := patch["max_bet"].(float64); ok && v > 0 {
		row.MaxBet = v
	}
	if v, ok := patch["maintenance_message"].(string); ok {
		row.MaintenanceMessage = v
	}
	if row.MinBet > row.MaxBet {
		return nil, errors.New("min_bet cannot exceed max_bet")
	}
